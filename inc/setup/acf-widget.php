...
class ACF_Widget {
	...
	public function register_cpt() {
		...
		$labels = [
			'name'               => _x( 'Page widgets', 'post type general name', 'nof' ),
			'singular_name'      => _x( 'Page widget', 'post type singular name', 'nof' ),
	...
	public static function body_class_setup( $classes ) {
		...
		if ( $meta ) {
			foreach ($meta as $wgt) {
				$classes[] = str_replace('wgt_', 'pw-', $wgt);
			}
		}
	...
	private static function get_slug( $wgt ) {
		...
		if ( get_key($wgt, 'acf_fc_layout') )
			return str_replace(['wgt_', '_' ], ['', '-'], $wgt['acf_fc_layout']);
	 ...
	 */
	private static function get_class( $wgt, $slug ) {
		...
		ob_start();
			...
			if (count($content)) :
				foreach ($content as $key => $wgt) :
					...
					$widget          = $wgt;
					$widget['name']  = $slug;
					$widget['class'] = self::get_class($wgt, $slug);

					ob_start();
						get_template_part( 'templates/widgets/wgt', $slug );
					echo ob_get_clean();
				endforeach;
			endif;
